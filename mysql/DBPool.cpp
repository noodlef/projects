...
int CDBManager::Init()
{
	config_file_reader config_file("dbproxyserver.conf");

	char* db_instances = config_file.get_config_name("DBInstances");
	...
	for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
		...
		char* db_host = config_file.get_config_name(host);
		char* str_db_port = config_file.get_config_name(port);
		char* db_dbname = config_file.get_config_name(dbname);
		char* db_username = config_file.get_config_name(username);
		char* db_password = config_file.get_config_name(password);
		char* str_maxconncnt = config_file.get_config_name(maxconncnt);
