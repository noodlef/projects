...
namespace muduo
{
	namespace detail
	{
		...
		struct local_time
		{
			...
			local_time(time_t offset, bool dst, int arrb)
...
struct time_zone::Data
{
	vector<detail::transition> transitions;
	vector<detail::local_time> localtimes;
	vector<string> names;
	string abbreviation;
};
...
namespace muduo
{
	namespace detail
	{
		...
		public:
			...
			{
				int32_t x = 0;
				size_t nr = ::fread(&x, 1, sizeof(int32_t), _fp);
			...
			{
				uint8_t x = 0;
				size_t nr = ::fread(&x, 1, sizeof(uint8_t), _fp);
		...
		bool readTimeZoneFile(const char* zonefile, struct time_zone::Data* data)
		{
			File f(zonefile);
			if (f.valid())
			{
