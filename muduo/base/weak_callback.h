...
namespace muduo
{
	...
	public:
		...
		weak_callback(const boost::weak_ptr<CLASS>& object,
		...
		void operator()() const
		{
			boost::shared_ptr<CLASS> ptr(_object.lock());
			if (ptr)
			{
				_function(ptr.get());
			}
	...
	private:
		...
		boost::weak_ptr<CLASS>                _object;
		boost::function<void(CLASS*)>         _function;
	};

	template<typename CLASS>
	weak_callback<CLASS> make_weak_callback(const boost::shared_ptr<CLASS>& object,
		void (CLASS::*function)())
	{
		return weak_callback<CLASS>(object, function);
	}
