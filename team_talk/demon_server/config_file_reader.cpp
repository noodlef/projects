...
{
	if (!m_load_ok) {
		mix_log("trying to get %s from a \
...
{
	if (!m_load_ok) {
		mix_log("trying to update %s to a \
				     noexsitent config file!", name);
		return -1;
	}
	map<string, string>::iterator it = m_config_map.find(name);
	...
	void config_file_reader::_load_config_file(const char* filename)
	{
		...
		if (!fp)
		{
			mix_log("can not open %s,errno = %d", filename, errno);
			return;
		}
		// 将配置信息读入 map 
		char buf[256];
		for (;;)
		{
			...
			if (!p) {
				if (ferror(fp))
					mix_log("load config file failed!");
				break;
			}
			// remove \n at the end
	...
	int config_file_reader::_update_config_file(const char*filename)
	{
		...
		if (fp == NULL)
		{
			mix_log("can't update config file!");
			return -1;
		}
		char szPaire[256];
